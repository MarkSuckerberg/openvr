...
class HadesDeviceDriver : public vr::ITrackedDeviceServerDriver
{
public:
	HadesDeviceDriver(CdataHandler &dh)
		: dataHandler(dh)
	{
	...
	EVRInitError Activate(vr::TrackedDeviceIndex_t unObjectId)
	{
		...
		vr::VRProperties()->SetStringProperty(m_ulPropertyContainer, Prop_SerialNumber_String, "000001");
		vr::VRProperties()->SetBoolProperty(m_ulPropertyContainer, Prop_WillDriftInYaw_Bool, false);
		vr::VRProperties()->SetStringProperty(m_ulPropertyContainer, Prop_ManufacturerName_String, "MARK");
	...
	HmdMatrix34_t CalcMatFromEuler(float yaw, float pitch, float roll, float IPDOffset)
	{
		...
		mat.m[0][0] = cos(yaw) * cos(roll);
		mat.m[0][1] = sin(pitch) * sin(yaw) * cos(roll) - cos(pitch) * sin(roll);
		mat.m[0][2] = cos(pitch) * sin(yaw) * cos(roll) + sin(pitch) * sin(roll);
		mat.m[0][3] = IPDOffset;
		mat.m[1][0] = cos(yaw) * sin(roll);
		mat.m[1][1] = sin(pitch) * sin(yaw) * sin(roll) + cos(pitch) * cos(roll);
		mat.m[1][2] = cos(pitch) * sin(yaw) * sin(roll) - sin(pitch) * cos(roll);
		mat.m[1][3] = 0;
		mat.m[2][0] = -sin(yaw);
		mat.m[2][1] = sin(pitch) * cos(yaw);
		mat.m[2][2] = cos(pitch) * cos(yaw);
		mat.m[2][3] = 0;
		return mat;
	}
...
public:
	...
	void SetTrackerIndex(int32_t trckIndex)
	{
...
{
public:
	EVRInitError Init(vr::IVRDriverContext *pDriverContext);
	void Cleanup();
...
private:
	...
	HadesDeviceDriver *m_pHmd = nullptr;
...
EVRInitError CServerDriver_Sample::Init(vr::IVRDriverContext *pDriverContext)
{
	...
	if (headlessMode)
	{
		...
		HMDConnected = true;
		m_pHmd = new HadesDeviceDriver(*dataHandler);
