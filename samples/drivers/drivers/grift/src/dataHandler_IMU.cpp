...
void CdataHandler::UpdateIMUPosition(_TrackingData &_data, V3Kalman &_k)
{
	...
	if (Vector3::Magnitude(_data.Velocity) > 100.f && orientationFilterInit == true)
...
{
	if (_data.RotationUserOffset.W == 0.f && _data.RotationUserOffset.Y == 0.f)
	{ // Don't try to use an enpty offset quaternion
...
void CdataHandler::UpdateDriftCorrection(_TrackingData &_data, float lowerTreshold, float upperTreshold, bool debug)
{
	...
	if (vm >= lowerTreshold && vm <= upperTreshold && im >= vm / psmsUpdateRate)
	{
		...
		if (std::abs(_data.RotationYawDriftOffset - rot.Y) > 2.5f)
		{
			rot.Y += M_PI;
		}
		...
		if (debug)
			DriverLog("[Debug] Drift correction: Final correction: %f, Noisy boi correction: %f", _data.RotationYawDriftOffset * 180.0 / M_PI, rot.Y * 180.0 / M_PI);
	}
	_data.IMUVelocity = Vector3::Zero();
}
